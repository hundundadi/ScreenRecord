...
static audio_context_t *sys_audio_ctx = NULL;
 ...
 */
audio_context_t *create_mic_audio_context(int api, int device)
{

    close_audio_context();

    my_audio_ctx = audio_init(api, device,1);
...
audio_context_t *create_sys_audio_context(int api, int device)
{

    close_audio_context();

    sys_audio_ctx = audio_init(api, device,0);

    if(sys_audio_ctx == NULL)
        fprintf(stderr, "deepin-camera: couldn't allocate audio context\n");

    return sys_audio_ctx;
}
/*
 * get audio context
 * args:
...
audio_context_t *get_sys_audio_context()
{
    if(!sys_audio_ctx)
        return NULL;

    /*force a valid number of channels*/
    if(audio_get_channels(sys_audio_ctx) > 2)
        audio_set_channels(sys_audio_ctx, 2);
...
void close_sys_audio_context()
{
    if(sys_audio_ctx != NULL)
        audio_close(sys_audio_ctx);
