 ...
 */
audio_context_t *audio_init(int api, int device,int deviceType)
{
	...
	switch(api)
	{
		...
		default:
            ret = audio_init_portaudio(audio_ctx,deviceType);
			break;
	}
...
audio_device_t* audio_get_device(audio_context_t *audio_ctx, int index)
{
    ...
    if(index >= audio_ctx->device_count)
	{
		fprintf(stderr, "AUDIO: (audio_get_device) bad index %i using %i\n",
            index, audio_ctx->device_count - 1);
        index = audio_ctx->device_count - 1;
	}
