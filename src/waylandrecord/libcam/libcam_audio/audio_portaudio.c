 ...
 * returns: error code (0 ok)
 */
static int audio_portaudio_list_devices(audio_context_t *audio_ctx,int deviceType)
{
	/*asserts*/
	assert(audio_ctx != NULL);
	...
	else
	{
		...
		for( it=0; it < numDevices; it++ )
		{
            ...
            printf( "it == getPortAudio()->m_Pa_GetDefaultInputDevice(): %d == %d \n", it , getPortAudio()->m_Pa_GetDefaultInputDevice()  );
            printf( "it == getPortAudio()->m_Pa_GetHostApiInfo( deviceInfo->hostApi )->defaultInputDevice: %d == %d\n", it , getPortAudio()->m_Pa_GetHostApiInfo( deviceInfo->hostApi )->defaultInputDevice);
            printf( "it == getPortAudio()->m_Pa_GetDefaultOutputDevice(): %d == %d\n", it ,getPortAudio()->m_Pa_GetDefaultOutputDevice()  );
            printf( "it == getPortAudio()->m_Pa_GetHostApiInfo( deviceInfo->hostApi )->defaultInputDevice: %d == %d\n", it , getPortAudio()->m_Pa_GetHostApiInfo( deviceInfo->hostApi )->defaultOutputDevice);
            printf( "audio_ctx->device_count = %d\n", audio_ctx->device_count);
            if(deviceType > 0){
                if( it == getPortAudio()->m_Pa_GetDefaultInputDevice())
                {
                    ...
                    defaultDisplayed = 1;
                    audio_ctx->device = audio_ctx->device_count;/*default index in array of input devs*/
                    audio_ctx->device_type = 1;/*default index in array of input devs*/
                }
                ...
                if (deviceInfo->maxInputChannels > 0)
                {
                    audio_ctx->device_count++;
                    /*add device to list*/
                    audio_ctx->list_devices = realloc(audio_ctx->list_devices, audio_ctx->device_count * sizeof(audio_device_t));
                    ...
                    /*fill device data*/
                    audio_ctx->list_devices[audio_ctx->device_count-1].id = it;
                    strncpy(audio_ctx->list_devices[audio_ctx->device_count-1].name, deviceInfo->name, 511);
                    strncpy(audio_ctx->list_devices[audio_ctx->device_count-1].description, deviceInfo->name, 255);
                    audio_ctx->list_devices[audio_ctx->device_count-1].channels = deviceInfo->maxInputChannels;
                    audio_ctx->list_devices[audio_ctx->device_count-1].samprate = deviceInfo->defaultSampleRate;
                    audio_ctx->list_devices[audio_ctx->device_count-1].high_latency = (double) deviceInfo->defaultHighInputLatency;
                    audio_ctx->list_devices[audio_ctx->device_count-1].low_latency = (double) deviceInfo->defaultLowInputLatency;
                }
            }else {
                /* Output devices (if it has output channels it's a capture device)*/
                if( it ==getPortAudio()->m_Pa_GetDefaultOutputDevice())
                {
                    ...
                    defaultDisplayed = 3;
                    audio_ctx->device = audio_ctx->device_count;
                    audio_ctx->device_type = 0;
                }
                ...
                if (deviceInfo->maxOutputChannels > 0)
                {
                    ...
                    strncpy(audio_ctx->list_devices[audio_ctx->device_count-1].name, deviceInfo->name, 511);
                    strncpy(audio_ctx->list_devices[audio_ctx->device_count-1].description, deviceInfo->name, 255);
                    audio_ctx->list_devices[audio_ctx->device_count-1].channels = deviceInfo->maxOutputChannels;
                    audio_ctx->list_devices[audio_ctx->device_count-1].samprate = deviceInfo->defaultSampleRate;
                    audio_ctx->list_devices[audio_ctx->device_count-1].high_latency = (double) deviceInfo->defaultHighOutputLatency;
                    audio_ctx->list_devices[audio_ctx->device_count-1].low_latency = (double) deviceInfo->defaultLowOutputLatency;
                }
            }
    ...
    audio_ctx->device_name = audio_ctx->list_devices[audio_ctx->device].name;
    printf( "api = %d\n", audio_ctx->api );
    printf( "audio_ctx->device_name = %s\n", audio_ctx->device_name );
    printf( "audio_ctx->device_count = %d\n", audio_ctx->device_count );
    printf( "audio_ctx->device = %d\n", audio_ctx->device );
    printf( "audio_ctx->channels = %d\n", audio_ctx->channels );
    printf( "audio_ctx->samprate = %d\n", audio_ctx->samprate );
    printf( "audio_ctx->latency = %f\n", audio_ctx->latency );
    printf( "audio_ctx->current_ts = %ld\n", audio_ctx->current_ts );
    printf( "audio_ctx->last_ts = %ld\n", audio_ctx->last_ts );
    printf( "audio_ctx->snd_begintime = %ld\n", audio_ctx->snd_begintime );
    printf( "audio_ctx->ts_drift = %ld\n", audio_ctx->ts_drift );
    printf( "audio_ctx->capture_buff = %f\n", audio_ctx->capture_buff );
    printf( "audio_ctx->capture_buff_size = %d\n", audio_ctx->capture_buff_size );
    printf( "audio_ctx->capture_buff_level[2] = %f\n", audio_ctx->capture_buff_level[2] );
    printf( "audio_ctx->stream_flag = %d\n", audio_ctx->stream_flag );
    printf( "==========================================\n");
    fflush(stdout);
    return 0;
}
 ...
 */
int audio_init_portaudio(audio_context_t* audio_ctx,int deviceType)
{
	/*assertions*/
	assert(audio_ctx != NULL);
    ...
    if(audio_portaudio_list_devices(audio_ctx,deviceType) != 0)
	{
...
void audio_set_portaudio_device(audio_context_t *audio_ctx, int index)
{
    ...
    printf( "index %d\n", index );
    printf( ">>>>>>>>>>>>>>>>>>>>>>>>>1 %d\n", audio_ctx->device );
//    if(index >= audio_ctx->device_count)
//        audio_ctx->device = audio_ctx->device_count - 1;
//	else if(index >= 0 )
//        audio_ctx->device = audio_ctx->device_count - 1;
    printf( ">>>>>>>>>>>>>>>>>>>>>>>>>2 %d\n", audio_ctx->device );
...
int audio_start_portaudio(audio_context_t *audio_ctx)
{
    ...
    printf("<<<<< audio_ctx->device_type = %d\n",audio_ctx->device_type);
fflush(stdout);
    ...
    if(audio_ctx->device_type == 1){
        ...
        outputParameters.device = audio_ctx->list_devices[audio_ctx->device].id;
        outputParameters.channelCount = 2;
        outputParameters.sampleFormat = paFloat32; /*sample_t - float*/

        outputParameters.suggestedLatency = audio_ctx->latency; /*DEFAULT_LATENCY_DURATION/1000.0;*/
