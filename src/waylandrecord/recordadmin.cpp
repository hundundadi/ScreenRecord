...
void RecordAdmin::initGStreamer()
{
    ...
    m_gstreamProcess = new GStreamProcess();
    m_gstreamProcess->app = m_pAppSrcOption;
    m_gstreamProcess->m_width = m_selectWidth;
    m_gstreamProcess->m_height = m_selectHeight;
    //m_gstreamProcess->init();
    m_gstreamProcess->start();
    qDebug() << __LINE__ << " RecordAdmin::initGStreamer end!";
}

void RecordAdmin::initGStreamer1()
{
    qDebug() << QDateTime::currentDateTime().toString(Qt::DateFormat::LocalDate) << __LINE__ << " RecordAdmin::initGStreamer start!";
...
//    bool bRet;
//    m_pInputStream = new OpenAudioInput();
//    bRet = m_pInputStream->initAudio(); //初始化采集设备
//    connect(m_pInputStream, SIGNAL(sendMicAudioProcessing(uchar *, uint)), this, SLOT(onRecordAudio(uchar *, uint)));
//    connect(m_pInputStream, SIGNAL(sendSysAudioProcessing(uchar *, uint)), this, SLOT(onRecordSysAudio(uchar *, uint)));
...
//    m_pOutputStream->close();
    m_cacheMutex.unlock();
    //m_gstreamProcess->stopRecordFrame();
    m_pInputStream->stop();
...
void RecordAdmin::onRecordAudio(uchar *data, uint size)
{
    //m_gstVideoWriter->writeMicAudio(data, size);
}
void RecordAdmin::onRecordSysAudio(uchar *data, uint size)
{
    //m_gstVideoWriter->writeAudio(data,size);
}
